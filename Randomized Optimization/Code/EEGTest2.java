...
//  TODO: Your dataset filename. Expected format is CSV
  private static final String FILENAME = "ABAGAIL/src/opt/test/EEG_data.csv"; //ABAGAIL/

  //  TODO: How many examples you have
  private static int num_examples = 12811; //12811

  //  TODO: How many attributes you have. This is (number_of_columns - 1)
  private static int num_attributes = 13;
  ...
  private static double maxLearningRate = 25;
  private static double minLearningRate = 0.0000001;
  ...
  private static int numberHiddenLayerNodes = 7; //TO DO: Add value once find NNParams has run
...
//  TODO: Set this to false if you retained the best SA params from a previous run
  private static final boolean shouldFindSAParams = false; //NOT RUN YET
   ...
   */
  private static final boolean shouldFindGAParams = false; //NOT RUN YET
  private static double[] populationRatios = {0.05, 0.10, 0.15, 0.20, 0.25, 0.3, 0.35};
  private static double[] mateRatios = {0.01, 0.02, 0.04, 0.06};
  private static double[] mutateRatios = {0.01, 0.02, 0.04, 0.06};
  ...
  private static int[] iterations = {10, 100, 250, 500, 1000, 1500, 2000};
...
//      This is how Weka does it: # of attributes + # of classes / 2
      ...
      int[] hiddenLayerNodes = {3, 5, weka_technique, 9, 11, 13, 20, 25, 30, 40, 60, 80, 100};
      trainIterations = 2500;
    ...
    System.out.println("Algorithm, Number of Iterations, Train Error, Test Error, Time");

//     for (int i  = 0; i < iterations.length; i++) {
//       trainIterations = iterations[i];
// //    Now determine the NN performance. Results are simply printed out.
//       //System.out.println(trainIterations);
...
//    Run actual GA with best params here
    for (int m  = 0; m < iterations.length; m++) {
      trainIterations = iterations[m];
  ...
  public static void determineNNParams(int trainIterations, int[] hiddenLayerNodes) {
    ...
    System.out.println("NN Parameters");
    System.out.println(initialLearningRate+","+maxLearningRate+","+minLearningRate+","+backprop_threshold);
...
//    Convert nanoseconds to seconds
    ...
    System.out.println("BP,"+trainIterations+","+trainError+","+testError+","+time_elapsed);
...
//    Convert nanoseconds to seconds
    time_elapsed /= Math.pow(10,9);
    //System.out.printf("Time Elapsed: %s s %n", df.format(time_elapsed));
    System.out.println("RHC,"+trainIterations+","+trainError+","+testError+","+time_elapsed);
...
//      Find the average error for this network configuration
        ...
        System.out.println("DeterminingSAParameters,"+temp+","+cooling+","+errors[x][y]);
...
//      Find the average error for this configuration
          ...
          System.out.println("DeterminingGAParameters,"+population+","+mate+","+mutate+","+errors[x][y][z]);
